...
export function ServiceCard({ item }: { item: ServiceExperience }) {
  const name = item.author.name?.trim() || "ExperienceHub user";
  const initials = name
    .split(/\s+/)
    .slice(0, 2)
    ...
    <article className="rounded-xl border bg-card p-4">
      ...
      <div className="mt-3 flex flex-wrap gap-1.5 text-xs">
        <span className="rounded-full bg-muted px-2 py-0.5 font-medium">
          {labelFor(CHANNEL, item.channel)}
        </span>
        <span className="rounded-full bg-muted px-2 py-0.5 font-medium">
          {labelFor(RESPONSE_TIME, item.responseTime)}
        </span>
        <span className="rounded-full bg-muted px-2 py-0.5 font-medium">
          {labelFor(REPAIR_OUTCOME, item.repairOutcome)}
        </span>
        <span className="rounded-full bg-muted px-2 py-0.5 font-medium">
          Warranty: {labelFor(SERVICE_WARRANTY, item.warranty)}
        </span>
      </div>

      {item.issue && (
        <p className="mt-2 text-xs text-muted-foreground">Serviced: {item.issue}</p>
      )}
      {item.comment && (
      ...
      <footer className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
        {item.technicianRating != null && (
          <span className="inline-flex items-center gap-1">
            ...
            Technician {item.technicianRating}/5
          </span>
        )}
        ...
        {item.cost != null && (
          <span>{item.cost === 0 ? "Free" : `৳${item.cost.toLocaleString()}`}</span>
        )}
      </footer>
    </article>
  );
